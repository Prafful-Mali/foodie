...
class PaymentService:    
    ...
    @transaction.atomic
    def verify_payment_signature(self, payment: Payment, payment_id: str, signature: str) -> bool:
        try:
        ...
        if payment.status == PaymentStatus.CREATED:
            ...
            payment.save(update_fields=["payment_id", "status", "verified_at", "updated_at"])
    ...
    @transaction.atomic
    def store_webhook(self, event_id: str, event_type: str, payload: Dict) -> Tuple[WebhookEvent, bool]:
    ...
    def process_webhook(self, webhook_event: WebhookEvent) -> Dict:
        webhook_event = WebhookEvent.objects.select_for_update().get(id=webhook_event.id)
        ...
        payment = Payment.objects.filter(order_id=order_id).select_related('subscription', 'tenant').first()
        if not payment:
    ...
    def handle_captured(self, payment: Payment, payment_entity: Dict):
        ...
        logger.info(f"Payment {payment.order_id} captured - PREMIUM GRANTED to tenant {tenant.id}")
