...
class TestUserCRUDIntegration:
    ...
    def test_create_user_as_admin(self, mock_task, admin_client, tenant):
        ...
        assert response.data["message"] == "User created successfully. An email has been sent to them to set their password."
    ...
    def test_create_user_as_regular_user(self, authenticated_client):
        response = authenticated_client.post(reverse("user-list"), data={"email": fake.email()})
    ...
    def test_retrieve_user_as_admin(self, admin_client, tenant):
        ...
        response = admin_client.get(reverse("user-detail", kwargs={"pk": other_user.pk}))
        assert response.status_code == 200
        assert response.data["email"] == other_user.email

    def test_retrieve_self(self, authenticated_client, user):
        response = authenticated_client.get(reverse("user-detail", kwargs={"pk": user.pk}))
        assert response.status_code == 200
        assert response.data["email"] == user.email

    def test_retrieve_other_user_as_regular(self, authenticated_client, tenant):
        other_user = UserFactory(tenant=tenant)
        response = authenticated_client.get(reverse("user-detail", kwargs={"pk": other_user.pk}))
        assert response.status_code == 403
    ...
    def test_delete_user_as_admin(self, mock_task, admin_client, tenant):
        ...
        response = admin_client.delete(reverse("user-detail", kwargs={"pk": user_to_delete.pk}))
    ...
    def test_delete_other_user_as_regular(self, authenticated_client, tenant):
        other_user = UserFactory(tenant=tenant)
        response = authenticated_client.delete(reverse("user-detail", kwargs={"pk": other_user.pk}))
        assert response.status_code == 403

    def test_delete_self_as_regular(self, authenticated_client, user):
        response = authenticated_client.delete(reverse("user-detail", kwargs={"pk": user.pk}))
        assert response.status_code == 204
